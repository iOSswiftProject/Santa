...
class DBInterface: NSObject {
    ...
    func selectMountainLog() -> [Mountain]{
        ...
        let query = "SELECT NAME, HEIGHT, LATITUDE, LONGTITUDE, DEPTH1, DEPTH2, PEAK, ISFAVORITE, ISVISIT, MOUNTAIN.ID, DATE, LOG.ID FROM MOUNTAIN, LOG WHERE MOUNTAIN.ID = LOG.MOUNTAINID ORDER BY DATE DESC;"
        var queryStatment: OpaquePointer? = nil
